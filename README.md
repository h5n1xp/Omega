...
5. The primary target for emulation is AmigaOS legal applications, therefore the hardardware emulation does not need to be as precise as UAE, and ultimately I don't *plan* to support anything more advanced than ECS... 
...
5. The display is based upon whatever the DMA has fetched for thr frame, if no bitplane data is fetched, nothing is displayed, my earlier design would just draw colour00 pixels, which is more "correct" from a visual behaviour point of view, but I felt it wasn't really adding anything... I might be wrong.
